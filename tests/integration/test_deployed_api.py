...
class TestDeployedAPI:
    ...
    def test_post_missing_name(self):
        ...
        assert response.status_code == 400, \
            f"Should reject missing name, got {response.status_code}"
    ...
    def test_post_invalid_email(self):
        ...
        assert response.status_code == 400, \
            f"Should reject invalid email, got {response.status_code}"
    ...
    def test_post_message_too_short(self):
        ...
        assert response.status_code == 400, \
            f"Should reject short message, got {response.status_code}"
        
        error_data = response.json()
        assert 'error' in error_data, "Error response should contain 'error' field"
        assert 'message' in error_data['error'].lower() or 'character' in error_data['error'].lower(), \
    ...
    def test_post_spam_detection(self):
        ...
        assert response.status_code == 400, \
            f"Should reject spam message, got {response.status_code}"
    ...
    def test_response_time_acceptable(self, valid_contact_data):
        ...
        assert response_time < 5.0, \
            f"API response too slow: {response_time:.2f}s (should be < 5s)"
